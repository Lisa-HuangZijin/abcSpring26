...
let lasttouchSend = 0;
let lastBeadSave = 0;
let fallenBeadsDirty = false; // 掉落珠子被推动后标记，触发提前保存

let chainPalettes = {};
let chainPhotos = {}; // photoURL → p5 Image（照片珠用，以 URL 为 key 避免 chainIndex 复用导致图片错乱）
...
function rebuildchains(serverchains, serverOrphanedBeads) {
  ...
  // server の orphanedBeads を復元（evict 済み chain の落下珠）
  // chains に存在する chainIndex は sd.fallenBeads で復元されるので除外し重複を防ぐ
...
function maybeSaveBeads() {
  const interval = fallenBeadsDirty ? 2000 : 2000;
  if (millis() - lastBeadSave < interval) return;
  lastBeadSave = millis();
  fallenBeadsDirty = false;
  // 按 chainIndex 分组，每组单独 emit
  let groups = {};
  for (let b of fallenBeads) {
    let ci = b.renderData.chainIndex;
    if (ci === undefined) continue;
    if (b.renderData.evicted) continue; // 已被 evict，不再同步
...
function handleInteractions() {
  ...
  if (isInteracting) {
    ...
    // 推掉落的珠子matterjs
    for (let b of fallenBeads) {
      let d = dist(tx, ty, b.position.x, b.position.y);
      if (d < interactRadius) {
        let angle = atan2(b.position.y - ty, b.position.x - tx);
        let strength = (interactRadius - d) * 0.00008;
        Body.applyForce(b, b.position, {
          x: Math.cos(angle) * strength + (tx - ptx) * 0.00003,
          y: Math.sin(angle) * strength + (ty - pty) * 0.00003,
        });
        fallenBeadsDirty = true;
      }
    }
  ...
  for (let id in remotetouchs) {
    ...
    //推掉落的珠子（其他用户 touch）
    for (let b of fallenBeads) {
      let d = dist(rt.x, rt.y, b.position.x, b.position.y);
      if (d < interactRadius) {
        let angle = atan2(b.position.y - rt.y, b.position.x - rt.x);
        let strength = (interactRadius - d) * 0.00008;
        Body.applyForce(b, b.position, {
...
// 接收其他客户端推动掉落珠子后的位置同步
socket.on("remote-fallen-beads-update", function ({ chainIndex, beads }) {
  // 找到对应 chainIndex 的本地物理体
  const localBeads = fallenBeads.filter(
    (b) => b.renderData.chainIndex === chainIndex && !b.renderData.evicted,
  );
  if (localBeads.length === 0 || !beads || beads.length === 0) return;

  // 用 row 做匹配，把远端位置同步过来
  const beadByRow = {};
  for (let saved of beads) beadByRow[saved.row] = saved;

  for (let b of localBeads) {
    const saved = beadByRow[b.renderData.row];
    if (!saved) continue;
    let restoredY = windowHeight - saved.yFromBottom;
    Body.setPosition(b, { x: saved.x, y: restoredY });
    Body.setAngle(b, saved.angle || 0);
    Body.setVelocity(b, { x: saved.vx || 0, y: saved.vy || 0 });
  }
});
...
(function () {
  ...
  if (myUserId) {
    // 既存ユーザー：intro を隠して珠帘を表示
    if (overlay) overlay.classList.add("hidden");
    socket.emit("hello", { userId: myUserId });
    return;
  }

  // 新規ユーザー（id なし）：intro を表示
  if (overlay) {
    overlay.classList.remove("hidden");
    overlay.classList.add("visible");
  }
  // intro-anim も一緒に表示（frame0 背景）
...
// ── "Add my chain" button ─────────────────────────────────────────────────────
const MAX_CHAINS = 5; // 与 server.js 保持一致
